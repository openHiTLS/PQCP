...
/**
 * FrodoKEM密钥封装机制（KEM）演示
 */
static int32_t FrodoKEMDemo(void)
{
    printf("\n=== FrodoKEM密钥封装机制(KEM)演示 ===\n\n");
    int32_t ret = -1;
    CRYPT_EAL_PkeyCtx *deCtx = NULL;
    int32_t cipherLen = 9752;
    int8_t cipher[9752] = {0};
    int32_t sharekeyLen = 16;
    int8_t sharekey[16] = {0};
    int32_t sharekey2Len = 16;
    int8_t sharekey2[16] = {0};
    int32_t val = PQC_ALG_ID_FRODOKEM_640_SHAKE;
    uint8_t pubdata[9616];
    BSL_Param pub[2] = {
        {CRYPT_PARAM_FRODOKEM_PUBKEY, BSL_PARAM_TYPE_OCTETS, pubdata, sizeof(pubdata), 0},
        BSL_PARAM_END
    };
    ...
    deCtx = CRYPT_EAL_ProviderPkeyNewCtx(NULL, CRYPT_PKEY_FRODOKEM, CRYPT_EAL_PKEY_KEM_OPERATE,
        "provider=pqcp");
    if (deCtx == NULL) {
        printf("create ctx failed.\n");
        goto EXIT;
    ...
    if (ret != CRYPT_SUCCESS) {
        printf("set encaps key failed : %d.\n", ret);
        goto EXIT;
    }
