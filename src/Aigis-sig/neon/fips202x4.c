...
extern void KeccakP1600times4_PermuteAll_24rounds(void *s);
...
void shake512_absorb4x(__m256i* s,
    ...
    unsigned long long mlen)
{
    keccak_absorb4x(s, SHAKE512_RATE, m0, m1, m2, m3, mlen, 0x1F);
}
...
void shake512_squeezeblocks4x(unsigned char* h0,
    ...
    __m256i* s)
{
    keccak_squeezeblocks4x(h0, h1, h2, h3, nblocks, SHAKE512_RATE, s);
}
...
{
    unsigned int i;
    unsigned long nblocks = hlen / SHAKE512_RATE;
    unsigned char t[4][SHAKE512_RATE];
    __m256i s[25];

    shake512_absorb4x(s, m0, m1, m2, m3, mlen);
    shake512_squeezeblocks4x(h0, h1, h2, h3, nblocks, s);

    h0 += nblocks * SHAKE512_RATE;
    h1 += nblocks * SHAKE512_RATE;
    h2 += nblocks * SHAKE512_RATE;
    h3 += nblocks * SHAKE512_RATE;
    hlen -= nblocks * SHAKE512_RATE;

    if (hlen) {
        shake512_squeezeblocks4x(t[0], t[1], t[2], t[3], 1, s);
        for (i = 0; i < hlen; ++i) {
            h0[i] = t[0][i];
            h1[i] = t[1][i];
            h2[i] = t[2][i];
