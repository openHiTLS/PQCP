...
{
    for (int32_t attempt = 0; attempt < 10; attempt++) {    // Maximum number of encoding retries when searching for a valid error vector of weight
        memset_s(e, params->nBytes, 0, params->nBytes);
        CRYPT_ERROR ret = FixedWeightVector(e, params->n, params->t, params);
        if (ret != PQCP_SUCCESS) {
            continue;
        }
...
static CRYPT_ERROR BuildVectorAndDecoding(uint8_t *e, const uint8_t *c0, const CMPrivateKey *sk, const McelieceParams *params)
{
    ...
    size_t cbLen = (((2 * params->m - 1) * MCELIECE_Q / 2) + 7) / 8;
    if (sk->controlbits == NULL || sk->controlbitsLen != cbLen) {
        BSL_SAL_FREE(v);
