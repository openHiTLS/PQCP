...
/*************************************************
* Name:        shake512_absorb
*
* Description: Absorb step of the SHAKE512 XOF.
*              non-incremental, starts by zeroeing the state.
*
* Arguments:   - uint64_t *s:                     pointer to (uninitialized) output Keccak state
*              - const unsigned char *input:      pointer to input to be absorbed into s
*              - unsigned long long inputByteLen: length of input in bytes
**************************************************/
void shake512_absorb(uint64_t* s, const unsigned char* input, unsigned long long inputByteLen)
{
    keccak_absorb(s, SHAKE512_RATE, input, inputByteLen, 0x1F);
}

/*************************************************
* Name:        shake512_squeezeblocks
*
* Description: Squeeze step of SHAKE128 XOF. Squeezes full blocks of SHAKE128_RATE bytes each.
*              Modifies the state. Can be called multiple times to keep squeezing,
*              i.e., is incremental.
...
{
    keccak_squeezeblocks(output, nblocks, s, SHAKE512_RATE);
}
...
void shake512(unsigned char* output, unsigned long long outlen,
    const unsigned char* input, unsigned long long inlen)
{
    uint64_t s[25];
    unsigned char t[SHAKE512_RATE];
    unsigned long long nblocks = outlen / SHAKE512_RATE;
    size_t i;

    /* Absorb input */
    keccak_absorb(s, SHAKE512_RATE, input, inlen, 0x1F);

    /* Squeeze output */
    keccak_squeezeblocks(output, nblocks, s, SHAKE512_RATE);

    output += nblocks * SHAKE512_RATE;
    outlen -= nblocks * SHAKE512_RATE;

    if (outlen)
    {
        keccak_squeezeblocks(t, 1, s, SHAKE512_RATE);
        for (i = 0; i < outlen; i++)
            output[i] = t[i];
    }
}
...
{
  uint64_t s[25];
  unsigned char t[SHA3_128_RATE];
  size_t i;

  /* Absorb input */
  keccak_absorb(s, SHA3_128_RATE, input, inlen, 0x06);
...
{
	uint64_t s[25];
	unsigned char t[2*SHA3_512_RATE];
	size_t i;
