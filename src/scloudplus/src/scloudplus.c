...
int32_t SCLOUDPLUS_PKEKeygen(const SCLOUDPLUS_Para* para, uint8_t* pk, uint8_t* sk)
{
    ...
    const uint8_t* r1 = seed + 16;
    const uint8_t* r2 = seed + 48;
    ret = CRYPT_EAL_Randbytes(alpha, 32);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHAKE256, alpha, 32,NULL, 0, seed, &seedLen);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    ...
    memcpy_s(pk + para->pk_size - 16, 16, seedA, 16);
                              ...
                              uint8_t* ctx)
{
    ...
    uint32_t seedLen = 64;
    uint8_t seed[64];
    const uint8_t* seedA = pk + para->pk_size - 16;
    const uint8_t* r1 = seed;
    const uint8_t* r2 = seed + 32;
    ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHAKE256, r, 32,NULL, 0, seed, &seedLen);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
...
int32_t PQCP_SCLOUDPLUS_Gen(SCLOUDPLUS_Ctx* ctx)
{
    ...
    CRYPT_EAL_MdCTX* HCtx = CRYPT_EAL_MdNewCtx(CRYPT_MD_SHA3_256);
    if (HCtx == NULL)
    {
        ret = PQCP_MALLOC_FAIL;
        goto EXIT;
    }
    ret = CRYPT_EAL_MdInit(HCtx);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    ret = CRYPT_EAL_MdUpdate(HCtx, ctx->public_key, ctx->para->pk_size);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    ret = CRYPT_EAL_MdFinal(HCtx, ctx->private_key + ctx->para->pke_sk_size + ctx->para->pk_size, &hpkLen);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    memcpy_s(ctx->private_key + ctx->para->kem_sk_size - 32, 32, z, 32);

    CRYPT_EAL_MdFreeCtx(HCtx);
    return PQCP_SUCCESS;
EXIT:
    if (ctx->public_key != NULL)
    {
                               ...
                               uint8_t* sharedSecret, uint32_t* ssLen)
{
    ...
    uint8_t in[ctx->para->ss + 32 + 32 + 32];
    uint8_t* C = ciphertext;
    uint8_t* m = in;
    uint8_t* hpk = in + ctx->para->ss;
    uint8_t* r = in + ctx->para->ss + 32;
    uint8_t* k = in + ctx->para->ss + 32 + 32;
    uint32_t outLen = 32;
    ...
    outLen = 64;
    ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHA3_512, m, ctx->para->ss, hpk, 32, r, &outLen);
    if (ret != PQCP_SUCCESS)
    {
        return ret;
    }
    ...
    ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHAKE256, k, 32, C, ctx->para->ctx_size, sharedSecret, ssLen);
    if (ret != PQCP_SUCCESS)
    {
        return ret;
    }
                               ...
                               uint8_t* sharedSecret, uint32_t* ssLen)
{
    ...
    ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHA3_512, m1, ctx->para->ss, hpk, 32, r1, &outLen);
    if (ret != PQCP_SUCCESS)
    {
        goto EXIT;
    }
    ...
    if (bl == 0)
    {
        ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHAKE256, k1, 32, C, ctx->para->ctx_size, sharedSecret, ssLen);
    }
    else
    {
        ret = SCLOUDPLUS_MdFunc(CRYPT_MD_SHAKE256, ctx->private_key + ctx->para->kem_sk_size - 32, 32, C,
