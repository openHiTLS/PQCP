...
static int32_t SampleSparseTernaryVectorLight(void *mdCtx, uint8_t q, const uint8_t *in, uint32_t inLen, uint8_t *e,
                                              uint32_t eLen)
{
    uint32_t len = eLen / 2;
    while (1) {
        int32_t ret = CRYPT_EAL_MdSqueeze(mdCtx, e + len, len);
        if (ret != PQCP_SUCCESS) {
            return ret;
        }
...
static int32_t SampleSparseTernaryVector128(void *mdCtx, uint8_t q, const uint8_t *in, uint32_t inLen, uint8_t *e,
                                            uint32_t eLen)
{
    uint8_t flag;
    ...
    int32_t ret = CRYPT_EAL_MdSqueeze(mdCtx, r, 64);
    if (ret != PQCP_SUCCESS) {
        return ret;
    }
    t = 0;
    ...
    flag = 1;
    while (flag) {
        CRYPT_EAL_MdSqueeze(mdCtx, r, 64);
        if (ret != PQCP_SUCCESS) {
            return ret;
        }
        t = 0;
...
static int32_t SampleSparseTernaryVector256(void *mdCtx, uint8_t q, const uint8_t *in, uint32_t inLen, uint8_t *e,
                                            uint32_t eLen)
{
    uint8_t flag;
    ...
    uint16_t e2;
    memset_s(e, eLen, 0, eLen);
    int32_t ret = CRYPT_EAL_MdSqueeze(mdCtx, r, SHAKE256_RATE * 2);
    if (ret != PQCP_SUCCESS) {
        return ret;
    }
    t = 0;
    ...
    flag = 1;
    while (flag) {
        ret = CRYPT_EAL_MdSqueeze(mdCtx, r, SHAKE256_RATE);
        if (ret != PQCP_SUCCESS) {
            return ret;
        }
        t = 0;
                                            ...
                                            uint32_t eLen, int32_t algId)
{
    ...
    switch (algId) {
        case PQCP_POLAR_LAC_LIGHT:
            ret = SampleSparseTernaryVectorLight(mdCtx, q, in, inLen, e, eLen);
            break;
        case PQCP_POLAR_LAC_128:
            ret = SampleSparseTernaryVector128(mdCtx, q, in, inLen, e, eLen);
            break;
        case PQCP_POLAR_LAC_256:
            ret = SampleSparseTernaryVector256(mdCtx, q, in, inLen, e, eLen);
            break;
        default:
            break;
    }
